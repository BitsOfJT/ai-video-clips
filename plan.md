...
- [ ] Sidebar **Long-Form Editor** tab + full-width editor page shell.
- [ ] Content preset: **Talking Head** only (Podcast / Tutorial / Vlog deferred to Phase 8).
- [ ] Transcript panel synced to playhead; click word → seek; search transcript → jump.
- [ ] **Text-based cut**: select transcript range → delete → ripple-close gap on timeline.
- [ ] **Remove Filler Words** (um / uh / like) with preview + confirm.
- [ ] **Remove Long Silences** (thresholds: 0.5s / 1s / 2s).
- [ ] Single video track + waveform lane + playhead; split at playhead; snap to word boundaries.
- [ ] Ripple delete / ripple trim; undo/redo (50+ steps); J/K/L or 1x/1.5x/2x review speed.
- [ ] Chapter markers → copy/export YouTube-formatted list (`0:00 Intro` …; ≥3 chapters, ≥10s each).
- [ ] Auto caption track from transcript; **Clean YouTube** style; **export SRT / VTT** (sidecar, not burn-in by default).
- [ ] Audio: noise-reduction slider + **Normalize Loudness (-14 LUFS)** + per-clip volume envelope.
- [ ] Visual: basic color (exposure / contrast / saturation / white balance) + **Copy color to all clips**.
- [ ] Lower thirds (3 presets) + simple title card.
- [ ] Export: named **YouTube 1080p** preset (H.264 + AAC, 16:9); progress bar; thumbnail frame grab (PNG/JPG).
- [ ] YouTube Metadata Panel (title, description, tags, chapters paste) — pre-fill from existing AI/title fields where available.
- [ ] Project autosave of `timeline_json`.
